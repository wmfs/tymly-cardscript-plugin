class CreateTodoEntry {
  ...
  async run (event, context) {
    ...
    // if todo assigned to a team, userId is empty
    const userId = teamName ? null : context.userId
    ...
    if (event.id) todoEntry.id = event.id

    try {
      const doc = await this.todos.upsert(todoEntry, {})

      if (!event.id) todoEntry.id = doc.idProperties.id

      todoEntry.launches = todoEntry.launches.map(l => {
        if (l.input) l.input.TODO_ENTRY_ID = doc.idProperties.id
        return l
      })
