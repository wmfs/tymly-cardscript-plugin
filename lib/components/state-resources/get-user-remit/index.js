...
class GetUserRemit {
  ...
  get userInfo () { return this.services.userInfo }

  async run (event, context) {
    const userId = context.userId

    this.services.caches.userMemberships.delete(userId)
    this.services.caches.rolesFromUserId.delete(userId)
    await this.rbac.listUserRoles(userId)
    ...
    const promises = [
      findTodos(userId, this.todos, this.rbac, this.userInfo)
        .then(resultsObj => {
  ...
  findComponents (model, titleCol, filter = {}) {
    return model.find(filter)
      .then(results => {
        const resultsObj = {}
        results.forEach(r => { resultsObj[r[titleCol]] = r })
        return resultsObj
      })
  } // findComponents
  ...
  processComponents (userRemit, componentType, components, alreadyInClientManifest) {
    ...
    const arrToCompare = Array.isArray(alreadyInClientManifest)
      ? alreadyInClientManifest
      : Object.keys(alreadyInClientManifest)

    const namesToRemove = _.difference(arrToCompare, Object.keys(components))
